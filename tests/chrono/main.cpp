...
TEST_CASE("To ns from 1970", "")
{
    ...
    SECTION("Basic test")
    {
        ...
        cdf::_impl::scalar_to_ns_from_1970(input.data(), 2, output.data());
    ...
    SECTION("Vectorized against scalar")
    {
        ...
        cdf::_impl::scalar_to_ns_from_1970(inputs.data(), inputs.size(), expected_outputs.data());
        std::vector<int64_t> outputs(inputs.size());
        cdf::to_ns_from_1970(inputs.data(), inputs.size(), outputs.data());
