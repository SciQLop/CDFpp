...
std::size_t save_record(T& s, const U& writer);

template <typename writer_t, typename record_t, typename T>
inline std::size_t save_field(writer_t& writer, T& field)
{
    if constexpr (is_string_field_v<T>)
    {
        writer.write(field.values.data(), field.values.length());
        return writer.fill('\0', T::max_len - field.values.length());
    }
    else
    {
        ...
        {
            auto v = endianness::decode<endianness::big_endian_t>(field);
...
template <typename writer_t, typename record_t, typename T, typename... Ts>
